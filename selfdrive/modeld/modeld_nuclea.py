...
MODEL_PATHS = {
  ModelRunner.THNEED: Path(__file__).parent / 'models/tu_modelo.thneed',
  ModelRunner.ONNX: Path(__file__).parent / 'models/tu_modelo.onnx'}
...
def postprocess_image(output_tensor, original_image, classes, classes_color):
    ...
    for i in range(len(nms_boxes)):
        ...
        draw_bounding_box(resized_image, class_ids[index], scores[index], x, y, x_plus_w, y_plus_h, classes, classes_color)

    return resized_image

def draw_bounding_box(img, class_id, confidence, x, y, x_plus_w, y_plus_h, classes, classes_color):
    label = f'{classes[class_id]} ({confidence:.2f})'
    color = classes_color[class_id]
  ...
  while True:
    ...
    if sm.updated["liveCalibration"] and sm.seen['roadCameraState'] and sm.seen['deviceState']:
      ...
      if using_remote_camera:
        ...
        fcam = ecam = CameraConfig(640, 360, 1397.0)
