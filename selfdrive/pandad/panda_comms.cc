...
PandaMavlinkHandle::PandaMavlinkHandle(std::string serial)
  ...
  try {
    real_handle = std::make_unique<PandaUsbHandle>(serial);
    LOGW("connected to %s over USB", serial.c_str());
  } catch (std::exception &e) {
#ifndef __APPLE__
    real_handle = std::make_unique<PandaSpiHandle>(serial);
  ...
  int PandaMavlinkHandle::control_write(uint8_t bRequest, uint16_t wValue,
    ...
    case PandaEndpoints::SET_FAN_SPEED:
      this->real_handle->control_write(bRequest, wValue, wIndex, timeout);
      return 0;
  ...
  int PandaMavlinkHandle::control_read(uint8_t bRequest, uint16_t wValue,
    ...
    case PandaEndpoints::GET_FAN_SPEED:
      return this->real_handle->control_read(bRequest, wValue, wIndex, data,
                                             wLength, timeout);
      break;
  ...
  bool unpack_can_buffer(uint8_t *data, uint32_t &size,
    ...
    // move the overflowing data to the beginning of the buffer for the next
    // round
...
#define ISOTP_CONT_FRAME 2
  ...
  int PandaMavlinkHandle::bulk_write(unsigned char endpoint,
  ...
  int PandaMavlinkHandle::bulk_read(unsigned char endpoint, unsigned char *data,
    ...
    while (lk.owns_lock() && !this->msg_queue.empty()) {
      ...
      if ((std::get<2>(msg).size() + sizeof(can_header) + total_read) >=
          length) {
        break;
      }
      this->msg_queue.pop();
